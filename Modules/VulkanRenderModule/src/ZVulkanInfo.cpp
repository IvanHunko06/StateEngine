...
bool ZVulkanInfo::initializeServices()
{
    ...
    vulkanSurfaceProvider_             = &ServiceLocator::GetRequiredService<IVulkanSurfaceProvider>();
    vulkanCreateInstanceConfiguration_ = &ServiceLocator::GetRequiredService<VulkanCreateInstanceConfiguration>();
    overrideActiveVulkanDevice         = ServiceLocator::TryGetService<OverrideActiveVulkanDevice>();
...
bool ZVulkanInfo::initializeVolk()
{
    ...
    if (res != VK_SUCCESS) {
        ZLOG_ERROR("VulkanRenderModule") << "Failed to initialize Volk: "
                                         << vkResultTypeInfo_->ToString(&res, nullptr).c_str();
        return false;
    }
    return true;
}
void ZVulkanInfo::setupAllocationCallbacks()
{
    allocationCallbacks_ =
        VkAllocationCallbacks {.pUserData     = nullptr,
                               .pfnAllocation = [](void* pUserData, size_t size, size_t alignment,
                                                   VkSystemAllocationScope allocationScope) -> void* {
                                   return MemoryAllocator_AlignedAllocate(size, alignment);
                               },
                               .pfnReallocation = [](void* pUserData, void* pOriginal, size_t size, size_t alignment,
                                                     VkSystemAllocationScope allocationScope) -> void* {
                                   return MemoryAllocator_Realloc(pOriginal, size, alignment);
                               },
                               .pfnFree = [](void* pUserData, void* pMemory) { MemoryAllocator_Deallocate(pMemory); },
#ifndef _DEBUG
                               ...
                               .pfnInternalAllocation =
                                   [](void* pUserData, size_t size, VkInternalAllocationType allocationType,
                                      VkSystemAllocationScope allocationScope) {
                                       const TypeInfo* vkSystemAllocationScopeType =
                                           nullptr;  // GET_TYPE_INFO("VulkanRenderModule::VkSystemAllocationScope");
                                       if (vkSystemAllocationScopeType) {
                                           ZLOG_DEBUG("VulkanAllocationCallbacks")
                                               << "Vulkan internal allocation of size " << size
                                               << " bytes for allocation scope "
                                               << vkSystemAllocationScopeType->ToString(&allocationScope, nullptr)
                                                      .c_str()
                                               << ".";
                                       }
                                   },
                               .pfnInternalFree =
                                   [](void* pUserData, size_t size, VkInternalAllocationType allocationType,
                                      VkSystemAllocationScope allocationScope) {
                                       ...
                                       if (vkSystemAllocationScopeType) {
                                           ZLOG_DEBUG("VulkanAllocationCallbacks")
                                               << "Vulkan internal free of size " << size
                                               << " bytes for allocation scope "
                                               << vkSystemAllocationScopeType->ToString(&allocationScope, nullptr)
                                                      .c_str()
                                               << ".";
...
bool ZVulkanInfo::createVulkanInstance()
{
    VkApplicationInfo appInfo {
        ...
        .applicationVersion = VK_MAKE_VERSION(vulkanCreateInstanceConfiguration_->applicationVersion.major,
        ...
        .engineVersion      = VK_MAKE_VERSION(vulkanCreateInstanceConfiguration_->engineVersion.major,
    ...
    ZLOG_DEBUG("VulkanRenderModule") << "Enabling " << enabledExtensions.size() << " instance extensions...";
    ...
    if (res != VK_SUCCESS) {
        ZLOG_ERROR("VulkanRenderModule") << "Failed to create Vulkan instance: "
                                         << vkResultTypeInfo_->ToString(&res, nullptr).c_str();
        return false;
    }
...
bool ZVulkanInfo::enumeratePhysicalDevices()
{
    ...
    if (res != VK_SUCCESS) {
        if (vkResultTypeInfo_)
            ZLOG_ERROR("VulkanRenderModule") << "Failed to enumerate Vulkan physical devices in first call: "
                                             << vkResultTypeInfo_->ToString(&res, nullptr).c_str();
        return false;
    }
    if (deviceCount == 0) {
    ...
    if (res != VK_SUCCESS) {
        ZLOG_ERROR("VulkanRenderModule") << "Failed to enumerate Vulkan physical devices in second call: "
                                         << vkResultTypeInfo_->ToString(&res, nullptr).c_str();
        return false;
    }
    ...
    for (auto& rawHandle : rawHandles) {
        ...
        ZLOG_INFO("VulkanRenderModule") << "Vulkan Physical Device " << index - 1 << ": "
                                        << curPhysicalDevice.properties.deviceName
                                        << " Driver Version: " << curPhysicalDevice.properties.driverVersion
                                        << " API Version: " << VK_VERSION_MAJOR(curPhysicalDevice.properties.apiVersion)
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(rawHandle, &queueFamilyCount,
                                                 curPhysicalDevice.queueFamilyProperties.data());
        ...
        if (surfaceFormatsCount) {
            ...
            vkGetPhysicalDeviceSurfaceFormatsKHR(rawHandle, surface_, &surfaceFormatsCount,
                                                 curPhysicalDevice.surfaceFormats.data());
        }
        ...
        if (presentModesCount) {
            ...
            vkGetPhysicalDeviceSurfacePresentModesKHR(rawHandle, surface_, &presentModesCount,
                                                      curPhysicalDevice.presentModes.data());
        }
        ...
        if (res == VK_SUCCESS && extensionsCount) {
            ...
            res =
                vkEnumerateDeviceExtensionProperties(rawHandle, nullptr, &extensionsCount, extensionProperties.data());
            for (auto& extProp : extensionProperties) {
                curPhysicalDevice.supportedExtensions.insert(ZString(extProp.extensionName));
            }
        }
        else {
            ZLOG_ERROR("VulkanRenderModule")
                << "Failed to enumerate Vulkan device  " << index - 1
                << " extension properties: " << vkResultTypeInfo_->ToString(&res, nullptr).c_str();
        }
    }
    return true;
}
...
void ZVulkanInfo::warmUpAllDevices()
{
    ...
    if (SUCCEEDED(CreateDXGIFactory(__uuidof(IDXGIFactory), (void**)&factory))) {
        ...
        for (UINT i = 0; factory->EnumAdapters(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i) {
            ...
            ZLOG_DEBUG("VulkanRenderModule")
                << "Creating test D3D11 device to warm up adapter: " << wideToUTF8(desc.Description).c_str();
            ID3D11Device* dev        = nullptr;
            ID3D11DeviceContext* ctx = nullptr;
            D3D_FEATURE_LEVEL fl;
            HRESULT hr = D3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, nullptr, 0, D3D11_SDK_VERSION,
                                           &dev, &fl, &ctx);
...
ZString ZVulkanInfo::wideToUTF8(const wchar_t* wideString)
{
    ...
    int requiredSize = WideCharToMultiByte(CP_UTF8, 0, wideString, -1, nullptr, 0, nullptr, nullptr);
    ...
    WideCharToMultiByte(CP_UTF8, 0, wideString, -1, utf8Buffer.data(), requiredSize, nullptr, nullptr);
...
const VulkanPhysicalDevice& ZVulkanInfo::selectActivePhysicalDevice() const noexcept
{
    if (overrideActiveVulkanDevice) {
        ZLOG_DEBUG("VulkanRenderModule")
            << "Found OverrideActiveVulkanDevice configuration. Index of the requested device: "
            << overrideActiveVulkanDevice->newActiveDevice;
